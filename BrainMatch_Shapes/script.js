...
async function initializeProgressSystem() {
  try {
    ...
    gameManager = new GameManager({
      ...
      storageManager,
      validator,
      analyticsBridge: null, // level-wise analytics are submitted directly from script.js
      config: CONFIG
    });
...
function createAnalyticsRunId() {
  ...
  return `brainmatch_shapes_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
}
...
function startAnalyticsLevel(level) {
  ...
  if (!analyticsRunId || level === 1) {
    ...
    analytics.initialize('BrainMatch_Shapes', analyticsRunId);
