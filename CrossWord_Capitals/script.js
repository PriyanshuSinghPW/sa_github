document.addEventListener('DOMContentLoaded', () => {
    ...
    let checkAttempts = 0;
    let capitalLevels = [];
    const submittedCapitalLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_capitals_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getCapitalLevelXp(levelNumber, totalLevels) {
    ...
    function buildCapitalLevels(clues) {
        const allClues = [...clues.across, ...clues.down];
        return allClues.map((clue, index) => ({
            clue,
            levelNumber: index + 1,
            xp: getCapitalLevelXp(index + 1, allClues.length)
        }));
    }
    ...
    function startAnalyticsLevel(metadata) {
        ...
        submittedCapitalLevels.clear();
        checkAttempts = 0;
        analytics.initialize('CrossWord_Capitals', analyticsRunId);
        capitalLevels.forEach(({ clue, levelNumber, xp }) => {
            analytics.startLevel(levelNumber, { levelNumber });
            analytics.addRawMetric(`level_${levelNumber}_capital`, clue.answer.toUpperCase());
            analytics.addRawMetric(`level_${levelNumber}_country`, clue.clue);
        ...
        analytics.addRawMetric('capital_count', String(capitalLevels.length));
        ...
        console.log('[Analytics] Capital levels started:', { count: capitalLevels.length, runId: analyticsRunId });
        postAnalyticsDebug('levels_started', { count: capitalLevels.length, runId: analyticsRunId, title: metadata.title });
    }

    function submitCapitalLevel(levelInfo, metrics = {}) {
        const { clue, levelNumber, xp } = levelInfo;
        if (submittedCapitalLevels.has(levelNumber)) {
        ...
        submittedCapitalLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] Capital level submitted:', { level: levelNumber, runId: analyticsRunId, xp, capital: clue.answer });
        postAnalyticsDebug('submit_success', { level: levelNumber, runId: analyticsRunId, xpEarned: xp, capital: clue.answer });
        return payload;
    }

    function submitCompletedCapitals(metrics = {}) {
        let submittedCount = 0;
        capitalLevels.forEach(levelInfo => {
            if (!submittedCapitalLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitCapitalLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpTotal() {
        return capitalLevels.reduce((total, levelInfo) => {
            return total + (submittedCapitalLevels.has(levelInfo.levelNumber) ? levelInfo.xp : 0);
        }, 0);
    }
    ...
    async function startGame() {
        try {
            ...
            capitalLevels = buildCapitalLevels(currentPuzzleData.clues);
            initializeGame();
    ...
    function submitPuzzle() {
        ...
        const submitMetrics = {
            ...
            incorrect_items: incorrectCount,
            empty_items: emptyCount,
            accuracy_percent: accuracy.toFixed(1),
            time_elapsed_seconds: Math.floor(timeSinceStart / 1000)
        };
        const newlySubmittedCapitals = submitCompletedCapitals(submitMetrics);
        console.log('[Game] Newly completed capitals submitted:', newlySubmittedCapitals);

        if (allCorrect) {
            console.log('[Game] All answers correct! Calculating score...');
            clearInterval(timerInterval);
            const finalScore = getSubmittedXpTotal();
            console.log(`[Game] Final score: ${finalScore} / ${CROSSWORD_MAX_TOTAL_XP}`);
            scoreDisplayElement.textContent = finalScore;
            inputs.forEach(input => input.readOnly = true);
            successOverlay.classList.remove('hidden');
            console.log('[Game] All capital analytics levels submitted');
        } else {
    ...
    function completeCurrentPuzzleForTest() {
        ...
        checkCompletion();
        console.log('DEV: Auto-completing crossword capitals puzzle...');
        submitPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordCapitalsDevCompleteHandled) {
                return;
            }
            event.__crosswordCapitalsDevCompleteHandled = true;
    ...
    window.addEventListener('beforeunload', () => {
        if (levelStartTime > 0 && submittedCapitalLevels.size < capitalLevels.length) {
            postAnalyticsDebug('session_ended_incomplete', {
                completedCapitals: submittedCapitalLevels.size,
                totalCapitals: capitalLevels.length
            });
        }
    });
