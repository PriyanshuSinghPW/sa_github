document.addEventListener('DOMContentLoaded', () => {
    ...
    let checkAttempts = 0;
    let organLevels = [];
    const submittedOrganLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_our_organs_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getOrganLevelXp(levelNumber, totalLevels) {
    ...
    function buildOrganLevels(clues) {
        const allClues = [...clues.across, ...clues.down];
        return allClues.map((clue, index) => ({
            clue,
            levelNumber: index + 1,
            xp: getOrganLevelXp(index + 1, allClues.length)
        }));
    }
    ...
    function startAnalyticsLevels(metadata) {
        ...
        submittedOrganLevels.clear();
        checkAttempts = 0;
        analytics.initialize('CrossWord_Our_Organs', analyticsRunId);
        organLevels.forEach(({ clue, levelNumber, xp }) => {
            analytics.startLevel(levelNumber, { levelNumber });
            analytics.addRawMetric(`level_${levelNumber}_organ`, clue.answer.toUpperCase());
        ...
        analytics.addRawMetric('organ_count', String(organLevels.length));
        analytics.addRawMetric('max_total_xp', String(CROSSWORD_MAX_TOTAL_XP));
        levelStartTime = Date.now();
        console.log('[Analytics] Organ levels started:', { count: organLevels.length, runId: analyticsRunId });
        postAnalyticsDebug('levels_started', { count: organLevels.length, runId: analyticsRunId, title: metadata.title });
    }

    function submitOrganLevel(levelInfo, metrics = {}) {
        const { clue, levelNumber, xp } = levelInfo;
        if (submittedOrganLevels.has(levelNumber)) {
        ...
        submittedOrganLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] Organ level submitted:', { level: levelNumber, runId: analyticsRunId, xp, organ: clue.answer });
        postAnalyticsDebug('submit_success', { level: levelNumber, runId: analyticsRunId, xpEarned: xp, organ: clue.answer });
        return payload;
    }

    function submitCompletedOrgans(metrics = {}) {
        let submittedCount = 0;
        organLevels.forEach(levelInfo => {
            if (!submittedOrganLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitOrganLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpTotal() {
        return organLevels.reduce((total, levelInfo) => {
            return total + (submittedOrganLevels.has(levelInfo.levelNumber) ? levelInfo.xp : 0);
        }, 0);
    }
    ...
    async function startGame() {
        try {
            ...
            organLevels = buildOrganLevels(currentPuzzleData.clues);
            initializeGame();
    ...
    function submitPuzzle() {
        ...
        if (allCorrect) {
            clearInterval(timerInterval);
            const newlySubmittedOrgans = submitCompletedOrgans({
            ...
            console.log('[Analytics] Newly completed organs submitted:', newlySubmittedOrgans);
    ...
    function completeCurrentPuzzleForTest() {
        ...
        checkCompletion();
        console.log('DEV: Auto-completing organs puzzle...');
        submitPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordOurOrgansDevCompleteHandled) {
                return;
            }
            event.__crosswordOurOrgansDevCompleteHandled = true;
