document.addEventListener('DOMContentLoaded', () => {
    ...
    const submittedMicroorganismLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_microorganisms_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getMicroorganismLevelXp(levelNumber) {
    ...
    function buildAnswerLevels(puzzleData) {
        ...
        return allClues.map((clue, index) => {
            ...
            return {
                ...
                puzzleLevel,
                levelNumber,
                xp: getMicroorganismLevelXp(levelNumber)
            };
        });
    }
    ...
    function initializeAnalyticsRun() {
        ...
        analytics.initialize('crossword_microorganisms', analyticsRunId);
    ...
    function startAnalyticsLevelsForCurrentPuzzle(metadata) {
        ...
        currentAnswerLevels.forEach(({ clue, levelNumber, puzzleLevel, xp }) => {
            if (submittedMicroorganismLevels.has(levelNumber)) {
                return;
            }
        ...
        console.log('[Analytics] Microorganism answer levels started:', {
    ...
    function submitMicroorganismLevel(levelInfo, metrics = {}) {
        ...
        analytics.recordTask(
            levelNumber,
            `microorganism_answer_${levelNumber}`,
            clue.clue,
        ...
        submittedMicroorganismLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] Microorganism answer level submitted:', {
            level: levelNumber,
            puzzleLevel,
            runId: analyticsRunId,
            xp,
    ...
    function submitCompletedMicroorganismAnswers(metrics = {}) {
        let submittedCount = 0;
        currentAnswerLevels.forEach(levelInfo => {
            if (!submittedMicroorganismLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitMicroorganismLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpForPuzzle(puzzleLevel) {
        return currentAnswerLevels.reduce((total, levelInfo) => {
            return total + (
                levelInfo.puzzleLevel === puzzleLevel && submittedMicroorganismLevels.has(levelInfo.levelNumber)
                    ? levelInfo.xp
                    : 0
            );
        }, 0);
    ...
    async function startGame() {
        ...
        const savedProgress = localStorage.getItem('invisibleWorldProgress');
        if (savedProgress) {
    ...
    function saveProgress() {
        ...
        localStorage.setItem('invisibleWorldProgress', JSON.stringify(progress));
    }
    ...
    function initializeGame() {
        ...
        try {
            ...
            titleElement.textContent = `🔬 Invisible World`;
    ...
    function checkPuzzle() {
        ...
        if (allCorrect) {
            const timeTaken = Date.now() - levelStartTime;
            const newlySubmittedAnswers = submitCompletedMicroorganismAnswers({
                ...metrics,
            ...
            console.log('[Analytics] Newly completed microorganism answers submitted:', newlySubmittedAnswers);
    ...
    function showEndGameOverlay() {
        ...
        let perLevelHTML = '';
        Object.entries(allPuzzleAnswerCounts).forEach(([level, count]) => {
            const offset = getLevelOffset(Number(level));
            let levelXp = 0;
            for (let index = 1; index <= count; index++) {
                const levelNumber = offset + index;
                levelXp += submittedMicroorganismLevels.has(levelNumber) ? getMicroorganismLevelXp(levelNumber) : 0;
            }
            perLevelHTML += `<div>Level ${level}: <strong>${levelXp} XP</strong></div>`;
        });

        postAnalyticsDebug('game_completed', {
            runId: analyticsRunId,
            finalTotalXp: totalXP,
            submittedLevels: submittedMicroorganismLevels.size
        });
        ...
        document.getElementById('endgame-play-again').addEventListener('click', () => {
            ...
            analyticsRunId = '';
            submittedMicroorganismLevels.clear();
            initializeAnalyticsRun();
            updateXPDisplay();
    ...
    window.addEventListener('beforeunload', () => {
        if (analyticsRunId && levelStartTime > 0) {
            ...
            console.log('[Analytics] Session ended before completing all microorganism answers.');
        }
    });
    ...
    function completeCurrentPuzzleForTest() {
        ...
        console.log('DEV: Auto-completing microorganisms puzzle...');
        checkPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordMicroorganismsDevCompleteHandled) {
                return;
            }
            event.__crosswordMicroorganismsDevCompleteHandled = true;
