document.addEventListener('DOMContentLoaded', () => {
    ...
    let physicsLevels = [];
    const submittedPhysicsLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_physicswalla_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getPhysicsLevelXp(levelNumber, totalLevels) {
    ...
    function buildPhysicsLevels(clues) {
        const allClues = [...clues.across, ...clues.down];
        return allClues.map((clue, index) => ({
            clue,
            levelNumber: index + 1,
            xp: getPhysicsLevelXp(index + 1, allClues.length)
        }));
    }
    ...
    function startAnalyticsLevels(metadata) {
        ...
        submittedPhysicsLevels.clear();
        checkAttempts = 0;
        submitAttempts = 0;
        analytics.initialize('CrossWord_PhysicsWalla', analyticsRunId);
        physicsLevels.forEach(({ clue, levelNumber, xp }) => {
            analytics.startLevel(levelNumber, { levelNumber });
            analytics.addRawMetric(`level_${levelNumber}_concept`, clue.answer.toUpperCase());
        ...
        analytics.addRawMetric('physics_answer_count', String(physicsLevels.length));
        analytics.addRawMetric('max_total_xp', String(CROSSWORD_MAX_TOTAL_XP));
        levelStartTime = Date.now();
        console.log('[Analytics] Physics levels started:', { count: physicsLevels.length, runId: analyticsRunId });
        postAnalyticsDebug('levels_started', { count: physicsLevels.length, runId: analyticsRunId, title: metadata.title });
    }

    function submitPhysicsLevel(levelInfo, metrics = {}) {
        const { clue, levelNumber, xp } = levelInfo;
        if (submittedPhysicsLevels.has(levelNumber)) {
        ...
        submittedPhysicsLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] Physics level submitted:', { level: levelNumber, runId: analyticsRunId, xp, answer: clue.answer });
        postAnalyticsDebug('submit_success', { level: levelNumber, runId: analyticsRunId, xpEarned: xp, answer: clue.answer });
        return payload;
    }

    function submitCompletedPhysicsAnswers(metrics = {}) {
        let submittedCount = 0;
        physicsLevels.forEach(levelInfo => {
            if (!submittedPhysicsLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitPhysicsLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpTotal() {
        return physicsLevels.reduce((total, levelInfo) => {
            return total + (submittedPhysicsLevels.has(levelInfo.levelNumber) ? levelInfo.xp : 0);
        }, 0);
    }
    ...
    async function startGame() {
        try {
            ...
            physicsLevels = buildPhysicsLevels(currentPuzzleData.clues);
            initializeGame();
    ...
    function endGameByTimeUp() {
        ...
        postAnalyticsDebug('times_up', { runId: analyticsRunId, submittedLevels: submittedPhysicsLevels.size });
        console.log('[Analytics] Session ended before completing all physics answers.');
    }
    ...
    function submitPuzzle() {
        ...
        if (allCorrect) {
            ...
            const newlySubmittedAnswers = submitCompletedPhysicsAnswers({
            ...
            console.log('[Analytics] Newly completed physics answers submitted:', newlySubmittedAnswers);
    ...
    window.addEventListener('beforeunload', () => {
        if (analyticsRunId && levelStartTime > 0) {
            postAnalyticsDebug('session_left_incomplete', {
                ...
                submittedLevels: submittedPhysicsLevels.size
            });
            console.log('[Analytics] Session ended before completing all physics answers.');
        }
    });
    ...
    function completeCurrentPuzzleForTest() {
        ...
        checkCompletion();
        console.log('DEV: Auto-completing PhysicsWalla puzzle...');
        submitPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordPhysicsWallaDevCompleteHandled) {
                return;
            }
            event.__crosswordPhysicsWallaDevCompleteHandled = true;
