...
async function initializeGameManager() {
  try {
    ...
    gameManager = new GameManager({
      ...
      storageManager,
      validator,
      analyticsBridge: null, // analytics payloads are submitted directly through submitAnalyticsLevel().
      config: CONFIG,
    });
...
let analyticsRunId = '';
const analyticsSubmittedLevels = new Set();
...
function createAnalyticsRunId() {
  ...
  return `brainmatch_mathssquares_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
}
...
function getAnalyticsManager() {
  if (!analyticsManager && typeof AnalyticsManager !== 'undefined') {
...
function startAnalyticsLevel(level) {
  const analytics = getAnalyticsManager();
  if (!analytics) {
    postAnalyticsDebug('analytics_manager_missing', { level });
    return;
  }
  ...
  if (!analyticsRunId || level === 1 || isReplayAfterSubmit) {
    ...
    analytics.initialize('BrainMatch_MathsSquares', analyticsRunId);
  ...
  console.log('[Analytics] Level started:', { level, runId: analyticsRunId });
  postAnalyticsDebug('level_started', { level, runId: analyticsRunId });
}

function submitAnalyticsLevel(level, turns, xp) {
  const analytics = getAnalyticsManager();
  if (!analytics) {
    postAnalyticsDebug('submit_skipped_missing_manager', { level, turns, xp });
    return;
  }
  if (!analyticsRunId) {
    postAnalyticsDebug('submit_skipped_missing_run_id', { level, turns, xp });
    return;
  }
  if (analyticsSubmittedLevels.has(level)) {
    postAnalyticsDebug('submit_skipped_duplicate', { level, turns, xp, runId: analyticsRunId });
    return;
  }

  const timeTaken = Date.now() - (gameState.levelStartTime || Date.now());
  postAnalyticsDebug('campaign_win_before_submit', { level, turns, xp, timeTaken, runId: analyticsRunId });
  analytics.endLevel(level, true, timeTaken, xp);
  ...
  const payload = analytics.submitLevel(level, { runId: analyticsRunId });
  if (payload && payload.success === false) {
    console.error('[Analytics] Level submit rejected:', payload.errors);
    postAnalyticsDebug('submit_rejected', { level, turns, xp, runId: analyticsRunId, errors: payload.errors });
    return;
  }

  analyticsSubmittedLevels.add(level);
  try {
    window.parent.postMessage(payload, '*');
  } catch (_error) {
    // Bridge already attempted delivery; this supports the local harness.
  }
  console.log('[Analytics] Level submitted:', { level, runId: analyticsRunId, xp });
  postAnalyticsDebug('submit_success', { level, turns, xp, runId: analyticsRunId });
}
...
function handleCampaignWin() {
  ...
  totalCampaignXP += xp;
  submitAnalyticsLevel(level, gameState.turns, xp);
...
function completeCurrentCampaignLevelForTest() {
  if (!DEV_MODE) {
    console.log("DEV: Auto-complete ignored because DEV_MODE is off.");
    return;
  }

  if (gameContainer.classList.contains("hidden")) {
    console.log("DEV: Auto-complete ignored because game screen is not active.");
    return;
  }

  if (gameState.gameMode !== "campaign") {
    console.log("DEV: Auto-complete ignored because mode is not campaign:", gameState.gameMode);
    return;
  }
...
window.__completeLevelForTest = completeCurrentCampaignLevelForTest;

// --- [NEW] DEV FEATURE: AUTO-COMPLETE LEVEL ---
function handleDevCompleteKey(e) {
  if ((e.key && e.key.toLowerCase() === "c") || e.code === "KeyC") {
    if (e.__brainMatchDevCompleteHandled) {
      return;
    }
    e.__brainMatchDevCompleteHandled = true;
    e.preventDefault();
    completeCurrentCampaignLevelForTest();
  }
}
