document.addEventListener('DOMContentLoaded', () => {
    const ENABLE_DEV_COMPLETE_SHORTCUT = true;
    ...
    let checkAttempts = 0;
    let currencyLevels = [];
    const submittedCurrencyLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_currency_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getCurrencyLevelXp(levelNumber, totalLevels) {
    ...
    function buildCurrencyLevels(clues) {
        const allClues = [...clues.across, ...clues.down];
        return allClues.map((clue, index) => ({
            clue,
            levelNumber: index + 1,
            xp: getCurrencyLevelXp(index + 1, allClues.length)
        }));
    }
    ...
    function startAnalyticsLevels(metadata) {
        ...
        submittedCurrencyLevels.clear();
        checkAttempts = 0;
        analytics.initialize('CrossWord_Currency', analyticsRunId);
        currencyLevels.forEach(({ clue, levelNumber, xp }) => {
            analytics.startLevel(levelNumber, { levelNumber });
            analytics.addRawMetric(`level_${levelNumber}_currency`, clue.answer.toUpperCase());
        ...
        analytics.addRawMetric('currency_count', String(currencyLevels.length));
        analytics.addRawMetric('max_total_xp', String(CROSSWORD_MAX_TOTAL_XP));
        levelStartTime = Date.now();
        console.log('[Analytics] Currency levels started:', { count: currencyLevels.length, runId: analyticsRunId });
        postAnalyticsDebug('levels_started', { count: currencyLevels.length, runId: analyticsRunId, title: metadata.title });
    }

    function submitCurrencyLevel(levelInfo, metrics = {}) {
        const { clue, levelNumber, xp } = levelInfo;
        if (submittedCurrencyLevels.has(levelNumber)) {
        ...
        submittedCurrencyLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] Currency level submitted:', { level: levelNumber, runId: analyticsRunId, xp, currency: clue.answer });
        postAnalyticsDebug('submit_success', { level: levelNumber, runId: analyticsRunId, xpEarned: xp, currency: clue.answer });
        return payload;
    }

    function submitCompletedCurrencies(metrics = {}) {
        let submittedCount = 0;
        currencyLevels.forEach(levelInfo => {
            if (!submittedCurrencyLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitCurrencyLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpTotal() {
        return currencyLevels.reduce((total, levelInfo) => {
            return total + (submittedCurrencyLevels.has(levelInfo.levelNumber) ? levelInfo.xp : 0);
        }, 0);
    }
    ...
    async function startGame() {
        try {
            ...
            currencyLevels = buildCurrencyLevels(currentPuzzleData.clues);
            initializeGame();
    ...
    function submitPuzzle() {
        ...
        if (allCorrect) {
            ...
            const newlySubmittedCurrencies = submitCompletedCurrencies(submitMetrics);
            const finalScore = getSubmittedXpTotal();
            console.log('[Analytics] Newly completed currencies submitted:', newlySubmittedCurrencies);
    ...
    function completeCurrentPuzzleForTest() {
        ...
        checkCompletion();
        console.log('DEV: Auto-completing crossword currency puzzle...');
        submitPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordCurrencyDevCompleteHandled) {
                return;
            }
            event.__crosswordCurrencyDevCompleteHandled = true;
