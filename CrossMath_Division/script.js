document.addEventListener('DOMContentLoaded', async () => {
    ...
    const GAME_ID = 'crossmath_division';
    ...
    function onPuzzleComplete() {
        const timeTaken = Date.now() - levelStartTime;
        const levelXp = getLevelCompletionXp(currentLevel);
        ...
        if (analytics && isLevelActive) {
            ...
            analytics.endLevel(currentLevel, true, timeTaken, levelXp);
            ...
            if (payload && payload.success === false) {
                ...
                postAnalyticsDebug('submit_success', { runId: currentRunId, levelNumber: currentLevel, xpEarned: levelXp });
            }
            isLevelActive = false;
        }
    ...
    playAgainBtn.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            if (analytics) {
                ...
                levelStartTime = Date.now();
                isLevelActive = true;
                postAnalyticsDebug('level_started', { runId: currentRunId, levelNumber: currentLevel });
            }
        }, 300);
    });
