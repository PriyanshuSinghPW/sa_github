document.addEventListener('DOMContentLoaded', () => {
    ...
    let siUnitLevels = [];
    const submittedSiUnitLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_si_unit_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getSiUnitLevelXp(levelNumber, totalLevels) {
    ...
    function buildSiUnitLevels(clues) {
        const allClues = [...clues.across, ...clues.down];
        return allClues.map((clue, index) => ({
            clue,
            levelNumber: index + 1,
            xp: getSiUnitLevelXp(index + 1, allClues.length)
        }));
    }
    ...
    function startAnalyticsLevels(metadata) {
        ...
        submittedSiUnitLevels.clear();
        checkAttempts = 0;
        submitAttempts = 0;
        analytics.initialize('CrossWord_SI_Unit', analyticsRunId);
        siUnitLevels.forEach(({ clue, levelNumber, xp }) => {
            analytics.startLevel(levelNumber, { levelNumber });
            analytics.addRawMetric(`level_${levelNumber}_unit`, clue.answer.toUpperCase());
        ...
        analytics.addRawMetric('si_unit_answer_count', String(siUnitLevels.length));
        analytics.addRawMetric('max_total_xp', String(CROSSWORD_MAX_TOTAL_XP));
        levelStartTime = Date.now();
        console.log('[Analytics] SI unit levels started:', { count: siUnitLevels.length, runId: analyticsRunId });
        postAnalyticsDebug('levels_started', { count: siUnitLevels.length, runId: analyticsRunId, title: metadata.title });
    }

    function submitSiUnitLevel(levelInfo, metrics = {}) {
        const { clue, levelNumber, xp } = levelInfo;
        if (submittedSiUnitLevels.has(levelNumber)) {
        ...
        submittedSiUnitLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] SI unit level submitted:', { level: levelNumber, runId: analyticsRunId, xp, answer: clue.answer });
        postAnalyticsDebug('submit_success', { level: levelNumber, runId: analyticsRunId, xpEarned: xp, answer: clue.answer });
        return payload;
    }

    function submitCompletedSiUnitAnswers(metrics = {}) {
        let submittedCount = 0;
        siUnitLevels.forEach(levelInfo => {
            if (!submittedSiUnitLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitSiUnitLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpTotal() {
        return siUnitLevels.reduce((total, levelInfo) => {
            return total + (submittedSiUnitLevels.has(levelInfo.levelNumber) ? levelInfo.xp : 0);
        }, 0);
    }
    ...
    async function startGame() {
        try {
            const response = await fetch('puzzle.json?v=3');
            if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
            currentPuzzleData = await response.json();
            siUnitLevels = buildSiUnitLevels(currentPuzzleData.clues);
            initializeGame();
    ...
    function endGameByTimeUp() {
        ...
        postAnalyticsDebug('times_up', { runId: analyticsRunId, submittedLevels: submittedSiUnitLevels.size });
        console.log('[Analytics] Session ended before completing all SI unit answers.');
    }
    ...
    function submitPuzzle() {
        ...
        if (allCorrect) {
            ...
            const newlySubmittedAnswers = submitCompletedSiUnitAnswers({
            ...
            console.log('[Analytics] Newly completed SI unit answers submitted:', newlySubmittedAnswers);
    ...
    window.addEventListener('beforeunload', () => {
        if (analyticsRunId && levelStartTime > 0) {
            postAnalyticsDebug('session_left_incomplete', {
                ...
                submittedLevels: submittedSiUnitLevels.size
            });
            console.log('[Analytics] Session ended before completing all SI unit answers.');
        }
    });
    ...
    function completeCurrentPuzzleForTest() {
        ...
        checkCompletion();
        console.log('DEV: Auto-completing SI Unit puzzle...');
        submitPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordSiUnitDevCompleteHandled) {
                return;
            }
            event.__crosswordSiUnitDevCompleteHandled = true;
