...
function createAnalyticsRunId() {
  ...
  return `brainmatch_livingworld_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
}
...
function startAnalyticsLevel(level) {
  ...
  if (!analyticsRunId || level === 1) {
    ...
    analytics.initialize('BrainMatch_LivingWorld', analyticsRunId);
