document.addEventListener('DOMContentLoaded', () => {
    ...
    let shapeLevels = [];
    const submittedShapeLevels = new Set();
    ...
    function createRunId() {
        ...
        return `crossword_shapes_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
    ...
    function getShapeLevelXp(levelNumber, totalLevels) {
    ...
    function buildShapeLevels(clues) {
        const allClues = [...clues.across, ...clues.down];
        return allClues.map((clue, index) => ({
            clue,
            levelNumber: index + 1,
            xp: getShapeLevelXp(index + 1, allClues.length)
        }));
    }
    ...
    function startAnalyticsLevels(metadata) {
        ...
        submittedShapeLevels.clear();
        checkAttempts = 0;
        submitAttempts = 0;
        analytics.initialize('CrossWord_Shapes', analyticsRunId);
        shapeLevels.forEach(({ clue, levelNumber, xp }) => {
            analytics.startLevel(levelNumber, { levelNumber });
            analytics.addRawMetric(`level_${levelNumber}_term`, clue.answer.toUpperCase());
        ...
        analytics.addRawMetric('shape_answer_count', String(shapeLevels.length));
        analytics.addRawMetric('max_total_xp', String(CROSSWORD_MAX_TOTAL_XP));
        levelStartTime = Date.now();
        console.log('[Analytics] Shape levels started:', { count: shapeLevels.length, runId: analyticsRunId });
        postAnalyticsDebug('levels_started', { count: shapeLevels.length, runId: analyticsRunId, title: metadata.title });
    }

    function submitShapeLevel(levelInfo, metrics = {}) {
        const { clue, levelNumber, xp } = levelInfo;
        if (submittedShapeLevels.has(levelNumber)) {
        ...
        submittedShapeLevels.add(levelNumber);
        try {
            window.parent.postMessage(payload, '*');
        } catch (_error) {
            // Bridge already attempted delivery; this supports the local harness.
        }
        console.log('[Analytics] Shape level submitted:', { level: levelNumber, runId: analyticsRunId, xp, answer: clue.answer });
        postAnalyticsDebug('submit_success', { level: levelNumber, runId: analyticsRunId, xpEarned: xp, answer: clue.answer });
        return payload;
    }

    function submitCompletedShapeAnswers(metrics = {}) {
        let submittedCount = 0;
        shapeLevels.forEach(levelInfo => {
            if (!submittedShapeLevels.has(levelInfo.levelNumber) && isClueSolved(levelInfo.clue)) {
                submitShapeLevel(levelInfo, metrics);
                submittedCount++;
            }
        });
    ...
    function getSubmittedXpTotal() {
        return shapeLevels.reduce((total, levelInfo) => {
            return total + (submittedShapeLevels.has(levelInfo.levelNumber) ? levelInfo.xp : 0);
        }, 0);
    }
    ...
    async function startGame() {
        try {
            ...
            shapeLevels = buildShapeLevels(currentPuzzleData.clues);
            initializeGame();
    ...
    function endGameByTimeUp() {
        ...
        postAnalyticsDebug('times_up', { runId: analyticsRunId, submittedLevels: submittedShapeLevels.size });
        console.log('[Analytics] Session ended before completing all shape answers.');
    ...
    function submitPuzzle() {
        ...
        if (allCorrect) {
            ...
            const newlySubmittedAnswers = submitCompletedShapeAnswers({
            ...
            console.log('[Analytics] Newly completed shape answers submitted:', newlySubmittedAnswers);
            console.log(`[Analytics] Puzzle completed with score: ${finalScore} / ${CROSSWORD_MAX_TOTAL_XP}`);
            inputs.forEach(input => input.readOnly = true);
            successOverlay.classList.remove('hidden');
        } else {
            submitCompletedShapeAnswers({
                check_attempts: checkAttempts,
                submit_attempts: submitAttempts,
                accuracy_percent: accuracy,
                correct_cells: correctCount,
    ...
    window.addEventListener('beforeunload', () => {
        if (analyticsRunId && levelStartTime > 0) {
            postAnalyticsDebug('session_left_incomplete', {
                ...
                submittedLevels: submittedShapeLevels.size
            });
            console.log('[Analytics] Session ended before completing all shape answers.');
        }
    });
    ...
    function completeCurrentPuzzleForTest() {
        ...
        checkCompletion();
        console.log('DEV: Auto-completing Shapes puzzle...');
        submitPuzzle();
    }
    ...
    function handleDevCompleteKey(event) {
        if ((event.key && event.key.toLowerCase() === 'c') || event.code === 'KeyC') {
            if (event.__crosswordShapesDevCompleteHandled) {
                return;
            }
            event.__crosswordShapesDevCompleteHandled = true;
